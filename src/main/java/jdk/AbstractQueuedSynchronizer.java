            ...
            if (node == null) {                 // allocate; retry before enqueue
                ...
                if (!timed) {
                    System.out.println(new SimpleDateFormat("HH:mm:ss.SSS").format(new Date())
                            + " acquire: LockSupport.park 阻塞:" + Thread.currentThread().getName());

                    LockSupport.park(this);
                } else if ((nanos = time - System.nanoTime()) > 0L) {
                    System.out.println(new SimpleDateFormat("HH:mm:ss.SSS").format(new Date())
                            + " acquire: LockSupport.parkNanos 阻塞:" + Thread.currentThread().getName());
                ...
                System.out.println(new SimpleDateFormat("HH:mm:ss.SSS").format(new Date())
                        + " acquire: clearStatus:" + Thread.currentThread().getName());
                node.clearStatus();
